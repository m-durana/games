  ...
  {#key index}
    <div class="card" in:fly={{ y: 16, duration: 220 }}>
      ...
      <div class="scope-wrap">
        <RadarScope scenario={current.scenario} size={1200} rangeRings={[5, 10, 20]}>
...
<style>
  ...
  .atc-call h2 { font-size: 1rem; font-weight: 600; line-height: 1.4; margin: 0; }
  .atc-call {
    display: flex;
  ...
  .atc-bubble {
    position: relative;
    flex: 1;
    /* Solid bg (computed from translucent overlay on --surface) so the tail
       triangle below can fake the inner cutout cleanly. */
    background: #18242f;
  ...
  /* Speech-bubble tail pointing left toward the controller icon. Two
     overlapping triangles fake an outlined bubble: the outer (border colour)
     is shifted slightly outward to leave a 1px rim peeking around the
     inner (background colour) triangle. */
  .atc-bubble::before,
  .atc-bubble::after {
    content: '';
    position: absolute;
