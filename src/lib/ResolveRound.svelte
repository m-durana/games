<script lang="ts">
  ...
  interface Props {
    ...
    onFinish: (results: ResolveRoundResult[]) => void;
    onQuit: () => void;
  }
  ...
  const SESSION_KEY = sessionKey('resolve', difficulty);
  // svelte-ignore state_referenced_locally
  const PKEY = progressKey('resolve', difficulty);
  ...
  const initial = (() => {
    ...
    // svelte-ignore state_referenced_locally
    return { questions: buildResolveRound(difficulty), index: 0, results: [] as ResolveRoundResult[] };
  })();

  let questions: ResolveQuestion[] = $state(initial.questions);
  let index = $state(initial.index);
  let pickedIndex: number | null = $state(null);
  let committed = $state(false);
  let results: ResolveRoundResult[] = $state(initial.results);
  ...
  $effect(() => {
    ...
    recordProgress({
      key: PKEY,
      gameKind: 'resolve',
      label: `Conflict Resolution · ${difficulty}`,
      category: 'Radar',
      difficulty,
      currentIndex: index,
...
<header class="bar">
  ...
  <span class="meta">{score}/{RESOLVE_ROUND_LENGTH}</span>
</header>
...
<section class="round">
  {#key index}
    <div class="card" in:fly={{ y: 16, duration: 220 }}>
      <div class="card-head">
        <span class="mode-pill">Conflict Resolution</span>
      ...
      {#if showInfo}
        <div class="mode-info">
          <p><strong>You're the en-route controller.</strong> The system has flagged a converging pair predicted to lose separation. Pick the resolution that opens spacing without creating a new conflict with other traffic.</p>
          <p>Read the scope: other aircraft at similar altitudes block one or more of the four options. A climb is blocked by traffic above; a left vector is blocked by traffic on that side.</p>
          <p class="tip">Tip: Hard mode has two of the four options blocked. Use the speed-vector toggle (<strong>V</strong>) to project tracks forward.</p>
        </div>
      {/if}

      <div class="right-col">
        <div class="atc-call alarm">
          <img class="atc-icon" src="https://unpkg.com/lucide-static@0.469.0/icons/triangle-alert.svg" alt="" aria-hidden="true" />
          <div class="atc-bubble">
            <span class="bubble-tag">STCA</span>
            {current.atcCall}
          </div>
        </div>
      ...
      <div class="scope-wrap">
        <RadarScope scenario={current.scenario} size={1200} vectorMin={2}>
          {#each current.scenario.aircraft as ac (ac.id)}
            <AircraftBlip
              aircraft={ac}
              conflict={current.scenario.pairIds.includes(ac.id)}
            />
          {/each}
        </RadarScope>
      </div>
...
<style>
  ...
  .atc-call.alarm .atc-bubble { background: rgba(239, 68, 68, 0.14); border-color: rgba(239, 68, 68, 0.55); }
  .atc-call.alarm .atc-bubble::before { border-right-color: rgba(239, 68, 68, 0.55); }
  .atc-call.alarm .atc-bubble::after { border-right-color: rgba(239, 68, 68, 0.14); }
  .atc-call.alarm .bubble-tag { color: var(--bad); background: rgba(239, 68, 68, 0.15); }
  .atc-icon { width: 32px; height: 32px; flex-shrink: 0; margin-top: 4px; filter: invert(48%) sepia(78%) saturate(2475%) hue-rotate(338deg) brightness(98%) contrast(91%); }
