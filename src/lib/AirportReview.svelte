<script lang="ts">
  import { airports as airportList, fetchAirportImages, regionOf, regionLabel, type AirportEntry } from './airports-game';
  ...
  const current = $derived(airportList[index]);
  const entry = $derived<ReviewEntry>(
    state[current?.iata] ?? { approved: [], rejected: [] },
  );
  ...
  async function loadFor(a: AirportEntry) {
    loading = true;
    error = '';
    images = [];
    try {
      images = await fetchAirportImages(a);
    } catch (err) {
  ...
  function next() { if (index < airportList.length - 1) index += 1; }
...
<header class="head">
  ...
  <p>{airportsWithAnyApproval}/{airportList.length} airports with ≥1 approved · {totalApprovedPhotos} photos approved</p>
</header>
  ...
  <section class="reviewer">
    <div class="airline-head">
      <div>
        ...
        <span>{current.city} · {current.country}{regionOf(current.country) ? ` · ${regionLabel(regionOf(current.country)!)}` : ''} · {index + 1} / {airportList.length}</span>
      </div>
      <div class="nav">
        <button onclick={prev} disabled={index === 0}>← Prev airport</button>
        <button onclick={next} disabled={index >= airportList.length - 1}>Skip →</button>
      </div>
    </div>
    ...
    <select
      ...
      onchange={(e) => {
        ...
        const i = airportList.findIndex((a) => a.iata === id);
        if (i >= 0) index = i;
      }}
    >
      {#each airportList as a, i}
        {@const e = state[a.iata]}
        {@const ac = e?.approved?.length ?? 0}
        {@const rc = e?.rejected?.length ?? 0}
        <option value={a.iata}>
          {i + 1}. {a.name} ({a.iata}){ac > 0 ? ` · ${ac} good` : ''}{rc > 0 ? ` · ${rc} not good` : ''}{ac === 0 && rc === 0 ? ' · untouched' : ''}
        </option>
      {/each}
    </select>
    ...
    {:else if images.length === 0}
      <div class="empty">
        ...
        <button onclick={next}>Skip to next airport</button>
      </div>
    {:else if currentPhoto}
      <div class="stage" class:approved={isApproved} class:rejected={isRejected}>
        ...
        <button class="next-plane" onclick={next} disabled={index >= airportList.length - 1}>
          Next airport →
        </button>
      </div>
    {/if}
