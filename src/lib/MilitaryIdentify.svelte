<script lang="ts">
  ...
  interface SavedSession {
    ...
    totalScore: number;
    scores: number[];
    recorded: MilitaryIdentifyResult[];
  }
  ...
  const SESSION_KEY = sessionKey('militaryIdentify', difficulty);
  // svelte-ignore state_referenced_locally
  const PKEY = progressKey('militaryIdentify', difficulty);
  ...
  const initial = (() => {
    ...
    if (canResume) {
      return {
        answers: saved!.answerIds.map((id) => militaryById(id)!),
        index: saved!.index,
        stage: saved!.stage,
        picked: saved!.picked,
    ...
    return {
      ...
      totalScore: 0,
      scores: [] as number[],
      recorded: [] as MilitaryIdentifyResult[],
    };
  })();
  ...
  let recorded: MilitaryIdentifyResult[] = $state(initial.recorded);
  ...
  $effect(() => {
    ...
    recordProgress({
      key: PKEY,
      gameKind: 'militaryIdentify',
      label: `Military Identify · ${difficulty}`,
      category: 'Aircraft',
      difficulty,
      currentIndex: index,
  ...
  const comboGroups = $derived(buildComboGroups(pooledMilitary()));
  function buildComboGroups(list: MilitaryAircraft[]) {
  ...
  const maxScore = $derived(MILITARY_ROUND_LENGTH * stagePoints[0]);
          ...
          <div class="prompt-row">
            ...
            {#if stage < maxStage}
              ...
              <button class="btn-ghost hint-btn" onclick={nextHint}>
                {stage === 0 ? `Show maker${costLabel}` : stage === 1 ? `Show origin & role${costLabel}` : `Narrow to 4 choices${costLabel}`}
              </button>
            {/if}
          </div>
