<script lang="ts">
  ...
  const savedAtMount = (() => {
    ...
    const canResume =
      ...
      (saved.index > 0 || saved.guessIatas.length > 0 || saved.scores.length > 0);
...
<section class="round">
  ...
  {:else}
    {#key index}
      ...
      {#if showInfo}
        <p class="mode-info">Deduce a mystery airport from attribute feedback across country, region, hub alliance, traffic tier, runways, layout, and latitude band.</p>
      {/if}
...
<style>
  ...
  .board-head, .board-row {
    display: grid;
    grid-template-columns: minmax(104px, 1.05fr) repeat(var(--cols, 6), minmax(72px, 1fr));
    gap: 1px;
